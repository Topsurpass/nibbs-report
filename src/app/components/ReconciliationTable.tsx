...
export default function ReconciliationTable({ rows }: { rows: ReconRow[] }) {
  ...
  const shown =
    filter === "breaches"
      ? rows.filter((r) => r.jiveBreach || r.collateralBreach || r.presenceIssue)
      : rows;
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-4">
        <div>
          ...
          <p className="text-xs text-muted">
            {rows.length} banks · joined by 10-digit code (mirrors NIBBS_CHECKER)
          </p>
        </div>
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="sticky top-0 z-10 bg-surface/95 backdrop-blur">
            <tr className="text-left text-[11px] uppercase tracking-wide text-muted">
              ...
              <th className="px-3 py-3 text-right font-semibold">Collateral (C)</th>
              <th className="px-3 py-3 text-right font-semibold">Smartdet (H)</th>
              <th className="px-3 py-3 text-right font-semibold">Frame (O)</th>
              <th className="px-3 py-3 text-right font-semibold" title="H − O (must be 0)">
                Jive (H−O)
              </th>
              <th className="px-3 py-3 text-right font-semibold" title="C + H (must be ≥ 0)">
                Collat. (C+H)
              </th>
              <th
          ...
          <tbody>
            {shown.map((r, i) => {
              const rowBreach = r.jiveBreach || r.collateralBreach;
              return (
                <tr
                  key={r.code}
                ...
                >
                  <td className="px-4 py-2.5">
                    ...
                    <div className="tnum text-[11px] text-muted">{r.code}</div>
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-right">
                    <Amount value={r.jive} breach={r.jiveBreach} />
                  </td>
                  <td className="px-3 py-2.5 text-right">
                    <Amount value={r.collateralCheck} breach={r.collateralBreach} />
                  </td>
                  <td className="px-3 py-2.5">
                    <div className="flex items-center justify-center gap-1.5">
                      <PresenceDot ok={r.inMaster} label="Master" />
                      <PresenceDot ok={r.inFrame} label="HTML (Frame)" />
                      <PresenceDot ok={r.inSmartdet} label="eTranzact (Smartdet)" />
                    </div>
                  </td>
                </tr>
              );
