...
export default function BreachSection({ breaches, reportDate, onChange }: Props) {
  const [copied, setCopied] = useState(false);

  const copyTable = async () => {
    const { html, tsv } = buildBreachClipboard(breaches, reportDate);
    try {
  ...
  const cellBase =
    "border border-border px-3 py-2 text-sm align-middle";
  ...
  return (
    <div className="print-block overflow-hidden rounded-2xl border border-red-500/30 bg-surface shadow-sm">
      ...
      <div className="no-print flex flex-wrap items-center justify-between gap-3 border-b border-red-500/20 bg-red-500/5 px-5 py-4">
        <div>
          <h2 className="flex items-center gap-2 text-base font-semibold text-red-700 dark:text-red-300">
            <span aria-hidden>⚠</span> Breached Section for Escalation
          </h2>
          <p className="text-xs text-muted">
            {breaches.length} item(s) · edit Status/Reason, then copy straight into your reply
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() => downloadBreachExcel(breaches, reportDate)}
      ...
      <p className="no-print px-5 pt-3 text-[11px] text-muted">
        Tip: “Copy table” copies a formatted grid — paste it directly into an Outlook/Gmail
        reply or a new Excel sheet.
      </p>
      ...
      <div className="overflow-x-auto p-4">
        <table className="w-full min-w-[820px] border-collapse">
          <caption className="mb-0 border border-b-0 border-border bg-red-600 px-3 py-2 text-left text-sm font-bold text-white">
            Breached Section For Escalation — {reportDate.toLocaleDateString("en-GB")}
          </caption>
          <thead>
          ...
          <tbody>
            {breaches.map((b, i) => (
              <tr key={`${b.code}-${b.kind}`} className="odd:bg-black/[0.015] dark:odd:bg-white/[0.02]">
                <td className={cellBase}>
                  <div className="font-medium text-foreground">{b.name}</div>
                  <div className="tnum text-[11px] text-muted">{b.code}</div>
                </td>
                <td className={`${cellBase} tnum text-right text-foreground`}>
                  {formatNaira(b.netSettlementPosition)}
                </td>
                <td className={`${cellBase} tnum text-right text-foreground`}>
                  {b.collateral === null ? "—" : formatNaira(b.collateral)}
                </td>
                <td className={`${cellBase} tnum text-right font-semibold text-red-600 dark:text-red-400`}>
