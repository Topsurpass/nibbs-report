...
export async function POST(request: Request, ctx: Ctx) {
	...
	try {
		...
		const textLines = [
			...
			"",
			note ? `Message:\n${note}\n` : "",
			`Sent by ${auth.user.firstName} ${auth.user.lastName} via NIBBS Settlement Auditor.`,
		];
      ...
      <p style="margin:16px 0 0;font-size:12px;color:#9ca3af;">The full report is attached as ${escapeHtml(filename)}.<br/>Sent by ${escapeHtml(`${auth.user.firstName} ${auth.user.lastName}`)} via NIBBS Settlement Auditor.</p>
    </body></html>`;
		...
		return Response.json({ ok: true, to: recipient.email, ccCount: cc.length });
	} catch (err) {
